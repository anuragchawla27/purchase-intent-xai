...
def train_and_evaluate(name: str, model, preprocessor, X_train, X_test, y_train, y_test) -> dict:
    """
    ...
    logger.info("%s — PR-AUC: %.4f | Recall: %.4f | Accuracy: %.4f",
                name, results["pr_auc"], results["recall"], results["accuracy"])
...
def train_with_strategy(name: str, base_model, preprocessor, strategy: str,
    ...
    logger.info("%s [%s] — PR-AUC: %.4f | Recall: %.4f", name, strategy, results["pr_auc"], results["recall"])
    return results, pipeline
